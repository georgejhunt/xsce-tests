...
vboxmanage clonevm debian9-min --register --name debian9-test
vboxmanage  modifyvm debian9-test --natpf1 "ssh,tcp,,5022,,22"
vboxmanage startvm debian9-test --type headless

YMD=`date +"%y%m%d-%H:%M"`
scp -P 5022 ../../scripts/iiab.sh root@localhost:/root/iiab.sh
# copy the desired local_vars.yml
scp -P 5022 ../../scripts/local_vars.yml root@localhost:/root/local_vars.yml
# execute the following remotely on the VM
time ssh -p 5022 root@localhost '/root/iiab.sh > /root/output.log'
scp -P 5022 root@localhost:/root/output.log ./debian9-$YMD.log
